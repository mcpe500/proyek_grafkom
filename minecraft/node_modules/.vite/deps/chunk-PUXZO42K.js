...
// node_modules/@react-three/fiber/dist/index-80784ca9.esm.js
...
function query(fn, keys2, preload2 = false, config = {}) {
  ...
  const entry = {
    keys: keys2,
    equal: config.equal,
    promise: fn(...keys2).then((response) => entry.response = response).then(() => {
...
function createEvents(store) {
  ...
  function intersect(event, filter) {
    ...
    let hits = eventsObjects.flatMap((obj) => {
      const state2 = getRootState(obj);
...
function createLoop(roots2) {
  ...
  function advance2(timestamp, runGlobalEffects = true, state2, frame2) {
    if (runGlobalEffects)
      flushGlobalEffects("before", timestamp);
    if (!state2)
      roots2.forEach((root) => render$1(timestamp, root.store.getState()));
    else
...
function Portal({
  ...
  const inject = React.useCallback((rootState, injectState) => {
    ...
    Object.keys(rootState).forEach((key) => {
      if (privateKeys.includes(key) || rootState[key] !== injectState[key]) {
...
function useContextBridge() {
  ...
  traverseFiber(fiber, true, (node) => {
    ...
    if (context2 && context2 !== FiberContext && !contexts.includes(context2))
      contexts.push(wrapContext(context2));
  });
  for (const context2 of contexts) {
    const value = (_a = ReactCurrentDispatcher.current) == null ? void 0 : _a.readContext(context2);
    values.set(context2, value);
  }
 ...
 * LICENSE file in the root directory of this source tree.
 */
//# sourceMappingURL=chunk-PUXZO42K.js.map
